...
import { addItem } from '../redux/CartFilter'
  ...
  const Pizza ={
    ...
    return(
        ...
        <div className="pizza-block__selector">
          <ul>
          {types.map((value, i) => (
            <li key={i} onClick={() => setActiveIndexTypes(i)} className={activeIndexTypes === i ? 'active' : ''}>{value === 1 ? 'тонкое' : 'традиционное'}</li>
          ))}
          </ul>
          <ul>
          {sizes.map((value, i) => (
            <li key={i} onClick={() => setActiveIndexSizes(i)} className={activeIndexSizes === i ? 'active' : ''}>{value} см</li>
          ))}
          </ul>
        </div>
        <div className="pizza-block__bottom">
          <div className="pizza-block__price">от {price} ₽</div>
          <button onClick={() => dispatch(addItem(Pizza))} className="button button--outline button--add">
            <svg
              width="12"
              height="12"
              viewBox="0 0 12 12"
